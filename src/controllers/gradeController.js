...
exports.getGradebook = (req, res) => {
  ...
  db.all("SELECT * FROM subjects ORDER BY subject_name ASC", [], (err, allSubjects) => {
    ...
    db.all("SELECT * FROM students ORDER BY id DESC", [], (err, allStudents) => {
      ...
      db.all(
        "SELECT * FROM grades WHERE (subject_id = ? OR CAST(subject_id AS TEXT) = ?) AND term = ? AND session_year = ?",
        [parseInt(selectedSubjectId, 10), String(selectedSubjectId), selectedTerm, selectedSession],
        (err, gradeRows) => {
          const gradesMap = {};
          (gradeRows || []).forEach(g => {
...
exports.postSaveGrades = (req, res) => {
  ...
  const promises = rawStudentIds.map(sId => {
    return new Promise((resolve) => {
      ...
      db.get(
        "SELECT id FROM grades WHERE (student_id = ? OR CAST(student_id AS TEXT) = ?) AND (subject_id = ? OR CAST(subject_id AS TEXT) = ?) AND term = ? AND session_year = ?",
        [studentId, String(studentId), cleanSubjectId, String(cleanSubjectId), term, session_year],
        (err, existing) => {
          if (existing) {
            db.run(
  ...
  db.get("SELECT * FROM students WHERE id = ?", [studentId], (err, student) => {
    ...
    db.all("SELECT * FROM students", [], (err, allStudents) => {
      ...
      const sql = `
        ...
        FROM grades g 
        JOIN subjects s ON (g.subject_id = s.id OR CAST(g.subject_id AS TEXT) = CAST(s.id AS TEXT))
        WHERE (g.student_id = ? OR CAST(g.student_id AS TEXT) = ?) AND g.term = ? AND g.session_year = ?
      `;

      db.all(sql, [studentId, String(studentId), selectedTerm, selectedSession], (err, gradeRecords) => {
        let grandTotal = 0;
